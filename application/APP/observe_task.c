...
extern vmc_leg_t vmc_right;												//右腿vmc
...
void Observe_task(void)
{
  ...
  while(1)
	{  
		wr= -Chassis.BM_motor[0][0].speed_rad-INS.Gyro[0]+vmc_right.d_alpha;//右边驱动轮转子相对大地角速度，这里定义的是顺时针为正
		vrb=wr*0.090f+vmc_right.L0*vmc_right.d_theta*arm_cos_f32(vmc_right.theta)
				+vmc_right.d_L0*arm_sin_f32(vmc_right.theta);//机体b系的速度
		
		wl= -Chassis.BM_motor[1][0].speed_rad-INS.Gyro[0]+vmc_left.d_alpha;//左边驱动轮转子相对大地角速度，这里定义的是顺时针为正
		vlb=wl*0.090f+vmc_left.L0*vmc_left.d_theta*arm_cos_f32(vmc_left.theta)
				+vmc_left.d_L0*arm_sin_f32(vmc_left.theta);//机体b系的速度
		
		aver_v=(vrb-vlb)/2.0f;//车体速度
